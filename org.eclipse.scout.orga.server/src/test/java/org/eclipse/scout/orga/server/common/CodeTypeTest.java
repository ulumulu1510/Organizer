package org.eclipse.scout.orga.server.common;
...
public class CodeTypeTest {
	...
	public void testMaleFemaleCode() {
		...
		Assert.assertEquals("Unexpected ID for code type of male code", codeMale.getCodeType()
				.getId(), SexCodeType.ID);

		ICode<String> codeFemale = CODES.getCode(SexCodeType.Female.class);
		Assert.assertNotNull("Female code not found", codeFemale);
		Assert.assertEquals("Unexpected ID for code type of female code", codeFemale.getCodeType()
				.getId(), SexCodeType.ID);
	...
	public void testLocaleCodeType() {
		LocaleCodeType codeType = (LocaleCodeType) CODES.getCodeTypes(LocaleCodeType.class)
				.get(0);
		...
		Assert.assertEquals("Unexpected language tag for Canada (French) found", canadaFrenchTag,
				canadaFrenchCode.getId());

		Assert.assertEquals("Unexpected Locale for tag 'Canada (French)'", canadaFrench,
				LocaleCodeType.getLocale(canadaFrenchTag));
