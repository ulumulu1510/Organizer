package org.eclipse.scout.orga.server.code;
...
import org.eclipse.scout.orga.shared.code.ApplicationCodeFormData;
import org.eclipse.scout.orga.shared.code.ApplicationCodePageData;
import org.eclipse.scout.orga.shared.code.ApplicationCodePageData.ApplicationCodeRowData;
import org.eclipse.scout.orga.shared.code.ApplicationCodeUtility;
import org.eclipse.scout.orga.shared.code.IApplicationCodeService;
import org.eclipse.scout.orga.shared.code.IApplicationCodeType;
import org.eclipse.scout.orga.shared.code.ReadApplicationCodePermission;
import org.eclipse.scout.orga.shared.code.UpdateApplicationCodePermission;
...
public class ApplicationCodeService extends AbstractBaseService<Code, CodeRecord> implements IApplicationCodeService {
	...
	public ApplicationCodePageData getApplicationCodeTableData(Class<? extends IApplicationCodeType> codeTypeName) {
		...
		String locale = ServerSession.get()
				.getLocale()
				.toLanguageTag();
		String typeText = getText(codeType.getId(), locale);

		codeType.getCodes(false)
				.stream()
				.forEach(code -> {
	...
	private String getText(String key, String locale) {
		return BEANS.get(TextService.class)
				.getText(key, locale);
	}
	...
	public ApplicationCodeFormData load(ApplicationCodeFormData formData) {
		...
		String codeId = formData.getCodeId()
				.getValue();
		...
		if (codeId != null) {
			...
			formData.getCodeText()
					.setValue(getCodeText(codeId, code));
			formData.getOrder()
					.setValue(getCodeOrder(codeRecord, code));
			formData.getActive()
					.setValue(getCodeActive(codeRecord, code));
		} else {
			formData.getCodeId()
					.setValue(ApplicationCodeUtility.generateCodeId());
			formData.getOrder()
	...
	private String getCodeText(String codeId, ICode<String> code) {
		String text = BEANS.get(TextService.class)
				.getText(codeId, TextService.LOCALE_DEFAULT);
	...
	private ICodeRow<String> toCodeRow(ApplicationCodeFormData formData) {
		String id = formData.getCodeId()
				.getValue();
		String text = formData.getCodeText()
				.getValue();
		double order = getOrder(formData);
		String icon = null;
		boolean active = formData.getActive()
				.getValue();

		return new CodeRow<String>(id, text).withOrder(order)
				.withIconId(icon)
				.withActive(active);
	}

	private double getOrder(ApplicationCodeFormData formData) {
		if (formData.getOrder()
				.getValue() != null) {
			return (double) formData.getOrder()
					.getValue()
					.doubleValue();
		} else {
			return 0.0;
	...
	/**
	 * Returns true iff the dynamic code specified by the provided id and code
	 * type id exists.
	 */
	...
	public List<ICodeRow<String>> loadCodeRowsFromDatabase(String codeTypeId) {
		...
		return getCodeRecords(codeTypeId).stream()
				.map(code -> {
					String id = code.getId();
	...
	/**
	 * Loads all dynamic code rows from the database for the provided code type
	 * id.
	 */
	...
	/**
	 * Loads the dynamic code from the database specified for the provided code
	 * type id and code id.
	 */
