package org.eclipse.scout.orga.server.security;
...
public class UserAuthenticator {
	...
	/**
	 * Method invoked to handle an authentication request targeted to '/auth'
	 * URL.
	 * <p>
	 * If the user's password requires external verification, this
	 * authentication returns with 403 response code and with the HTTP header
	 * {@link IUserStoreCredentialVerifier#HTTP_HEADER_X_AUTH_STATUS} set to
	 * {@link IUserStoreCredentialVerifier#AUTH_EXTERNAL_PASSWORD_VERIFICATION_REQUIRED}.
	 * <p>
	 * However, because this class is of the characteristics of an Authenticator
	 * and not an AccessController, the request's HTTP session is not touched or
	 * changed, nor the filter-chain continued.
	 */
	protected void handleAuthRequest(final HttpServletRequest req, final HttpServletResponse resp)
			throws IOException, ServletException {
		...
		final int status = m_config.getCredentialVerifier()
				.verify(credentials.getKey(), credentials.getValue());
	 ...
	 */
	protected void handleForbidden(final int status, final HttpServletResponse resp)
			throws IOException, ServletException {
	 ...
	 * <p>
	 * The default implementation expects the credentials to be included in HTTP
	 * headers in the BASIC Authentication format.
	 */
	protected Entry<String, char[]> readCredentials(final HttpServletRequest req) {
		final String[] basicCredentials = BEANS.get(ServletFilterHelper.class)
				.parseBasicAuthRequest(req);
