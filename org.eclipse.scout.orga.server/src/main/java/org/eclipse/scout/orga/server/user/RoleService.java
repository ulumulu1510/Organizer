...
import org.eclipse.scout.orga.server.security.PermissionService;
...
import org.eclipse.scout.orga.shared.role.RoleFormData;
import org.eclipse.scout.orga.shared.role.RoleFormData.PermissionTable;
import org.eclipse.scout.orga.shared.role.RoleFormData.PermissionTable.PermissionTableRowData;
import org.eclipse.scout.orga.shared.role.RoleTablePageData;
import org.eclipse.scout.orga.shared.role.RoleTablePageData.RoleTableRowData;
...
public class RoleService extends AbstractBaseService<Role, RoleRecord> implements IRoleService {
	...
	/**
	 * Returns the set of permissions for the role specified by the provided
	 * name.
	 */
	public List<Permission> getPermissions(String role) {
		...
		return getContext().select(rpt.PERMISSION)
				.from(rpt)
				.where(rpt.ROLE_NAME.eq(role))
				.fetchStream()
				.map(record -> {
					String permissionId = rpt.field(rpt.PERMISSION)
							.getValue(record);
	...
	private void storeRolePermissions(RoleRecord role, List<String> permissions) {
		...
		context.deleteFrom(rpt)
				.where(rpt.ROLE_NAME.eq(roleName))
				.execute();

		// add new user roles
		permissions.stream()
				.forEach(permission -> context.executeInsert(new RolePermissionRecord(roleName, permission)));
	}
	...
	public AbstractTablePageData getPermissionTableData(SearchFilter filter) {
		...
		BEANS.get(PermissionService.class)
				...
				.stream()
				.forEach(permission -> {
					org.eclipse.scout.orga.shared.role.PermissionTablePageData.PermissionTableRowData row = pageData
							.addRow();
					String id = permission.getName();
					String group = permission.getPackage()
							.getName();
					row.setId(id);
	...
	private List<String> getRolePermissions(RoleFormData formData) {
		String role = formData.getRoleId()
				.getValue();

		return getPermissions(role).stream()
				.map(permission -> {
					return permission.getClass()
							.getName();
				})
				.collect(Collectors.toList());
	}
	...
	private void addPermissionRows(RoleFormData formData, List<String> permissions) {
		...
		BEANS.get(IPermissionService.class)
				...
				.forEach(permission -> {
					String id = permission.getName();
					String groupId = permission.getPackage()
							.getName();
