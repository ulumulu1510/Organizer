...
import org.eclipse.scout.orga.server.ServerProperties.SuperUserSubjectProperty;

/**
 * Central point to obtain run contexts with super user rights, and to get the
 * super user subject and session.
 * <p>
 * A super user run context is a {@link ServerRunContext} with a user that has
...
public class SuperUserRunContextProducer extends ServerRunContextProducer {
	 ...
	 * <p>
	 * This method delegates to {@link #produce()}, meaning that the subject
	 * specified is ignored.
	 *
	 * @param subject
	 *            is ignored, and
	 *            {@link SuperUserRunContextProducer#getSubject()} used instead.
	 * @see #produce()
	 */
	@Override
	 ...
	 * Produces a new {@link ServerRunContext} with super user rights.
	 * <p>
	 * This implies a new {@link ServerRunContext} initialized with the
	 * superuser's {@link Subject}, with a dedicated {@link RunMonitor} set, and
	 * the superuser's shared {@link IServerSession} set.
	 */
	public ServerRunContext produce() {
		...
		return superUserRunContext.withSession(session.setIfAbsentAndGet(new Callable<IServerSession>() {
	...
	public IServerSession getSession() {
		return produce().getSession(); // via provide(), because the session is
										// created on first usage.
	}

	/**
	 * Method invoked to create the superuser's {@link IServerSession} with data
	 * as specified by the given {@link ServerRunContext}.
	 */
	protected IServerSession createServerSession(final ServerRunContext serverRunContext) {
		return BEANS.get(ServerSessionProvider.class)
				.provide(serverRunContext.copy());
	}

	/**
	 * It is important that this platform listener is the last platform listener
	 * invoked because it is stopping the superuser session (thus having highest
	 * possible order).
	 * <p>
	 * Stopping the session when the platform stated is stopped (instead of
	 * stopping) isn't going to work because the BEANS.get (and others) cannot
	 * be used anymore when that platform is stopped.
	 */
	...
	public static class PlatformListener implements IPlatformListener {
		...
		public void stateChanged(final PlatformEvent event) throws PlatformException {
			if (event.getState() == State.PlatformStopping) {
				final IServerSession serverSession = BEANS.get(SuperUserRunContextProducer.class)
						.getSession();
