...
public class ScoutXlsxSpreadsheetAdapter extends AbstractService {
	...
	public File exportPage(String templateName, Locale locale, int startRow, int startCol, IPage<?> page)
			throws ProcessingException {
		return exportPage(templateName, locale, startRow, startCol, page, false);
	}

	public File exportPage(String templateName, int startRow, int startCol, IPage<?> page, boolean infoRowsAtBegin)
			throws ProcessingException {
		return exportPage(templateName, null, startRow, startCol, page, infoRowsAtBegin, false);
	}

	public File exportPage(String templateName, Locale locale, int startRow, int startCol, IPage<?> page,
			boolean infoRowsAtBegin) throws ProcessingException {
		return exportPage(templateName, locale, startRow, startCol, page, infoRowsAtBegin, false);
	}

	public File exportPage(String templateName, int startRow, int startCol, IPage<?> page, boolean infoRowsAtBegin,
			boolean writeDateAtEnd) throws ProcessingException {
		if (page instanceof IPageWithTable<?>) {
			return exportTablePage(templateName, null, startRow, startCol, (IPageWithTable<?>) page, infoRowsAtBegin,
					writeDateAtEnd);
		} else {
			return exportNodePage(templateName, null, startRow, startCol, (IPageWithNodes) page, infoRowsAtBegin,
					writeDateAtEnd);
		}
	}

	public File exportPage(String templateName, Locale locale, int startRow, int startCol, IPage<?> page,
			boolean infoRowsAtBegin, boolean writeDateAtEnd) throws ProcessingException {
		if (page instanceof IPageWithTable<?>) {
			return exportTablePage(templateName, locale, startRow, startCol, (IPageWithTable<?>) page, infoRowsAtBegin,
					writeDateAtEnd);
		} else {
			return exportNodePage(templateName, locale, startRow, startCol, (IPageWithNodes) page, infoRowsAtBegin,
					writeDateAtEnd);
		}
	}

	public File exportNodePage(String templateName, int startRow, int startCol, IPageWithNodes page,
			boolean infoRowsAtBegin, boolean writeDateAtEnd) throws ProcessingException {
		return exportNodePage(templateName, null, startRow, startCol, page, infoRowsAtBegin, writeDateAtEnd);
	}

	public File exportNodePage(String templateName, Locale locale, int startRow, int startCol, IPageWithNodes page,
			boolean infoRowsAtBegin, boolean writeDateAtEnd) throws ProcessingException {
		if (page == null) {
			throw new ProcessingException("No page found to export.");
		}
		String title = page.getCell()
				.getText();
		Object[][] csv;
		...
		for (int i = 0; i < page.getChildNodeCount(); i++) {
			...
			String text = page.getChildNodes()
					.get(i)
					.getCell()
					.getText();
			csv[i + 1][0] = text;
		...
		infoRowList.add(TEXTS.get("Path") + ": " + page.getTree()
				.getPathText(page));
	...
	public File exportTablePage(String templateName, int startRow, int startCol, IPageWithTable<?> page,
			boolean infoRowsAtBegin, boolean writeDateAtEnd) throws ProcessingException {
		return exportTablePage(templateName, null, startRow, startCol, page, infoRowsAtBegin, writeDateAtEnd);
	}

	public File exportTablePage(String templateName, Locale locale, int startRow, int startCol, IPageWithTable<?> page,
			boolean infoRowsAtBegin, boolean writeDateAtEnd) throws ProcessingException {
		String title = page.getCell()
				.getText();
		...
		if (page.getSearchFilter() != null) {
			String[] searchTree = page.getSearchFilter()
					.getDisplayTexts();
		...
		// table.getColumnFilterManager().getDisplayTexts();
	...
	public File exportTable(String templateName, Locale locale, int startRow, int startCol, String title, ITable table)
			throws ProcessingException {
		Object[][] csv = exportTableRowsAsCSV(table);
		// build verbose search text
		// info rows
	 ...
	 *            {@code true} if the info rows should be at the begin,
	 *            {@code false} if the should be at the end
	 * @param infoRows
	 *            Infos rows
	 * @return Created file
	 */
	public File exportCsv(String templateName, Locale locale, int startRow, int startCol, String title, Object[][] csv,
			boolean infoRowsAtBegin, String[] infoRows) {
		XlsxAdapter workbook = null;
		try {
			// init
			workbook = prepareWorkbook(templateName, locale);
			// fill
			workbook.fillCSV(startRow, startCol, title, infoRowsAtBegin ? infoRows : null,
					infoRowsAtBegin ? null : infoRows, csv, true);
	...
	public Object[][] exportTableRowsAsCSV(ITable table) {
		...
		if (table.getSelectedRowCount() >= 2) {
			csv = table.exportTableRowsAsCSV(table.getSelectedRows(), table.getColumnSet()
					.getVisibleColumns(), true, false, false);
		} else {
			csv = table.exportTableRowsAsCSV(table.getFilteredRows(), table.getColumnSet()
					.getVisibleColumns(), true, false, false);
		}
		return csv;
	}
